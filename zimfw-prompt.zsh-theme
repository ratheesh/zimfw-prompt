...
function _fmt_git_action() {
    ...
    case $action in
        rebase-i)   print "${s}%F{27}>%F{162}%BR%b%F{27}>%{$italic%}%F{1}rebase-i%{$reset%}${e}" ;;
        rebase-m)   print "${s}%F{1}%{$italic%}rebase-merge%{$reset%}${e}" ;;
        rebase)     print "${s}%F{1}%{$italic%}>R>rebase%{$reset%}${e}" ;;
...
function get_left_gitprompt_info() {
    ...
    if [[ -n $branch_name && ${#branch_name} -lt 25 ]]; then
        branch=("%F{8}🙧 %F{172} %f${is_worktree:-""}%{$italic%}%F{37}%25>…>${branch_name}%>>%F{142}${tag_at_current_commit:-""}%{$reset%}%F{8}🙥 %f")
        [[ -n $is_worktree ]] && print -n "${branch:-""} $(_fmt_git_action)" || print -n "${branch:+""} $(_fmt_git_action)"
    else
        branch=("%F{8}🙧 %F{172} %f${is_worktree:-""}%{$italic%}%F{37}%75>…>${branch_name}%>>%F{142}${tag_at_current_commit:-""}%{$reset%}%F{8}🙥 %f")
        print -n "${branch:-""} $(_fmt_git_action)"
    fi
}
